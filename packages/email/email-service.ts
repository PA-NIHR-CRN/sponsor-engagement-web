...
export class EmailService {
  ...
  sendEmail = async (data: EmailArgs) => {
    ...
    const htmlSource = fs.readFileSync(path.resolve(process.cwd(), `src/email/templates/${templateName}.html.hbs`), {
      encoding: EMAIL_CHARSET,
    })
    const textSource = fs.readFileSync(path.resolve(process.cwd(), `src/email/templates/${templateName}.text.hbs`), {
      encoding: EMAIL_CHARSET,
    })

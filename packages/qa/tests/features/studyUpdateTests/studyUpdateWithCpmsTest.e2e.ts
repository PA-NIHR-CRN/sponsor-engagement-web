...
const uniqueTarget = new Date().toISOString().slice(11, 19).replace(/:/g, '')
...
test.describe('Sponsor engagement study update with CPMS study validation @wip', () => {
  ...
  test.skip('proposed se change with cpms validation', async ({ browser }) => {
    ...
    await test.step(`And I see my expected study details in CPMS`, async () => {
      ...
      // await expect(cpmsPage.locator('#CurrentStudyRecord_ActualOpeningDate')).toHaveValue('12/06/2024 00:00:00')
      await expect(cpmsPage.locator('#CurrentStudyRecord_PlannedRecruitmentEndDate')).toHaveValue('12/06/2027 00:00:00')
      // await expect(cpmsPage.locator('#CurrentStudyRecord_ActualRecruitmentEndDate')).toHaveValue('12/06/2024 00:00:00')
    })

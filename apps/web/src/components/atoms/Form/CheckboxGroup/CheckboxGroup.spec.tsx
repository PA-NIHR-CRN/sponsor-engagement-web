...
test('Renders correctly without errors or hint', () => {
  ...
  const { getByLabelText, getByRole } = render(
    <CheckboxGroup defaultValue={defaultValue} errors={errors} label={label} name={name}>
      <Checkbox label="option1" value="option1" />
      <Checkbox label="option2" value="option2" />
      <Checkbox label="option3" value="option3" />
    </CheckboxGroup>
  )
  ...
  expect(radioElement).toHaveAttribute('type', 'checkbox')
